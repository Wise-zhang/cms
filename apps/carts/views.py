...
from carts.serializers import CartSerializer, CartGoodsSerializer, CartDeleteSerializer, CartSelectSerializer
...
class CartView(APIView):
    ...
    def get(self, request):
        ...
        if user.is_authenticated:
            ...
            list_cart_selected = redis_conn.smembers('cart_select_%s' % user.id)  # cart_select_1 = {1}   这里应该是元组

            # 拼装字典 cart_1 = {1: 2， 2：2}   cart_select_1 = {1}
            # {1:{'count':2, 'select':False}, 2:{'count':2, 'select':False}}
            cart = {}
            # 把redis中所有的hush和set中的全部数据查出来便利组装到一个字典中

            # sku_id:{'count':count,'select':False}
            for sku_id, count in dict_cart.items():
                cart[int(sku_id)] = {
                    'count': int(count),
                    'select': sku_id in list_cart_selected  # 相当于布尔值 不是对就是错
                }
            # 查询数据库数据返回序列化后的数据
            # 查询购物车中所有的商品  cart.keys() cart中所有的键
            skus = Goods.objects.filter(id__in=cart.keys())  # 得到的是查询集
            for sku in skus:  # 数据库查询到的是没有count和select这两个字段的，手动添加
                # 给sku对象新增两个字段: 商品数量和勾选状态
                sku.count = cart[sku.id]['count']
                sku.select = cart[sku.id]['select']
...
class CartSelectView(APIView):
    ...
    def put(self, request):
        ...
        user = request.user    # 只考虑登录状态下，登录状态保存在redis数据库中
        if user.is_authenticated:
            redis_conn = get_redis_connection('cart')
            cart = redis_conn.hgetall('cart_%s' % user.id)
            sku_id_list = cart.keys()
            if select:  # 全选
                redis_conn.sadd('cart_select_%s' % user.id, *sku_id_list)
            else:  # 取消全选
